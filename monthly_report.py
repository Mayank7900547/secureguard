...
def prepare_kaggle_fraud_transactions(df: pd.DataFrame, limit: int = 200) -> pd.DataFrame:
    """
    Convert a Kaggle creditcard.csv fraud rows into a display-friendly DataFrame.
    Uses real Time and Amount columns from the dataset.
    """
    out = df.copy().head(limit).reset_index(drop=True)

    # ── Real transaction IDs using original index ─────────────────────────────
    orig_indices = df.index[:limit]
    out["txn_id"] = [f"KGL{i+1:06d}" for i in orig_indices]

    # ── Real timestamps from Kaggle Time column (seconds from first txn) ──────
    # Time col = seconds elapsed since first transaction in dataset
    # Spread them over the last 30 days for display realism
    if "Time" in out.columns:
        # normalise Time so the spread covers ~30 days
        t_vals = out["Time"].astype(float)
        t_min, t_max = t_vals.min(), t_vals.max()
        span_seconds = 30 * 24 * 3600  # 30 days
        if t_max > t_min:
            normalised = (t_vals - t_min) / (t_max - t_min) * span_seconds
        else:
            normalised = t_vals
        base = datetime.now()
        timestamps = [base - timedelta(seconds=span_seconds) + timedelta(seconds=float(s))
                      for s in normalised]
    else:
        base = datetime.now()
        timestamps = [base - timedelta(days=30) + timedelta(hours=i)
                      for i in range(len(out))]

    out["date"] = [t.strftime("%d %b %Y") for t in timestamps]
    out["time"] = [t.strftime("%H:%M")    for t in timestamps]

    # ── Merchant — use Amount magnitude as a proxy for merchant type ──────────
    merchants_high = ["Crypto Exchange", "Gambling Site", "Forex Broker",
                      "Casino", "Wire Transfer", "Luxury Goods"]
    merchants_mid  = ["Electronics Store", "Online Shopping", "Travel Booking",
                      "Hotel", "Car Rental"]
    merchants_safe = ["Grocery Store", "Petrol Station", "Restaurant",
                      "Pharmacy", "Supermarket", "Clothing Store"]

    rng = random.Random(99)  # fixed seed per row for reproducibility
    def pick_merchant(amount):
        if amount > 500:   return rng.choice(merchants_high)
        elif amount > 100: return rng.choice(merchants_mid)
        else:              return rng.choice(merchants_safe)

    amt_col = out["Amount"] if "Amount" in out.columns else pd.Series([50.0]*len(out))
    out["merchant"] = [pick_merchant(float(a)) for a in amt_col]

    # ── Location — seed per row so same transaction always gets same city ─────
    cities = ["Mumbai", "Delhi", "Bangalore", "London", "Dubai",
              "Singapore", "New York", "São Paulo", "Lagos", "Hong Kong",
              "Frankfurt", "Amsterdam", "Bucharest", "Kyiv", "Jakarta"]
    out["location"] = [cities[int(out["txn_id"].iloc[i][3:]) % len(cities)]
                       for i in range(len(out))]
    ...
    if "Predicted_Fraud" in out.columns:
        ...
        out["flagged"] = True  # these are fraud rows by definition

    if "Risk_Level" in out.columns:
        out["risk_level"] = out["Risk_Level"]
    else:
        # Use Amount to grade severity
        def risk_from_amount(a):
            if a > 500:   return "High Risk"
            elif a > 100: return "Moderate Risk"
            else:         return "High Risk"  # low-amount fraud is still high risk
        out["risk_level"] = [risk_from_amount(float(a)) for a in amt_col]

    # ── Amount from real CSV ──────────────────────────────────────────────────
    out["amount"] = amt_col.values

    out["flag_reason"] = "Confirmed fraud — Kaggle creditcard.csv (Class=1)"
...
def generate_kaggle_fraud_report(
    ...
    If results_df (from Real-time Detection predict_batch) is provided,
    uses those ML-scored rows directly so both tabs show identical data.
    Otherwise falls back to filtering Class=1 rows from df_full.
    Returns (txn_df, pdf_bytes).
    """
    if results_df is not None and len(results_df) > 0:
        # Use the actual ML results from Real-time Detection
        # Filter to predicted fraud rows, sorted by probability
        if "Predicted_Fraud" in results_df.columns:
            fraud_rows = results_df[results_df["Predicted_Fraud"] == 1].copy()
        elif "Class" in results_df.columns:
            fraud_rows = results_df[results_df["Class"] == 1].copy()
        else:
            fraud_rows = results_df.copy()

        if "Fraud_Probability" in fraud_rows.columns:
            fraud_rows = fraud_rows.sort_values("Fraud_Probability", ascending=False)

        sample = fraud_rows.head(n_sample).reset_index(drop=True)
    else:
        # Fallback: filter Class=1 from raw CSV
        if "Class" in df_full.columns:
            fraud_rows = df_full[df_full["Class"] == 1].copy()
            if len(fraud_rows) == 0:
                fraud_rows = df_full.copy()
        else:
            fraud_rows = df_full.copy()
        sample = fraud_rows.head(n_sample).reset_index(drop=True)

    txn_df    = prepare_kaggle_fraud_transactions(sample, limit=n_sample)
    pdf_bytes = generate_monthly_report(profile, txn_df, period=period)
    return txn_df, pdf_bytes
