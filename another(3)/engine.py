...
class FraudEngine:
    ...
    def predict_with_explanation(self, input_df: pd.DataFrame):
        ...
        ax  = fig.subplots()
        colors = ["#f43f5e" if v > 0 else "#38bdf8" for v in sv]
        ...
        buf = io.BytesIO()
        fig.savefig(buf, format="png", dpi=110, facecolor="#1e293b")
        buf.seek(0)
    ...
    # ── Scaling analysis ──────────────────────────────────────────────────────
    def run_scaling_analysis(self, custom_df=None, custom_sizes=None):
...
class KaggleFraudEngine:
    ...
    def predict_with_explanation(self, input_df: pd.DataFrame):
        ...
        fig = Figure(figsize=(10, 6))
        ax  = fig.subplots()
        colors = ["#f43f5e" if v > 0 else "#38bdf8" for v in top_sv]
    ...
    def get_global_explanation(self):
        ...
        bars = ax.barh(np.arange(len(KAGGLE_FEATURES)),
                       mean_shap[sorted_idx], color="#818cf8", alpha=0.85)
        for i, bar in enumerate(bars):
            if sorted_idx[i] in top2:
                bar.set_color("#f43f5e")
